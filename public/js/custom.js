...
$(document).ready(function () {
  feather.replace(); // Store initial image size
  ...
  requestAnimationFrame(setImageSize); // Populate thumbnails

  $('.zooma-main').children().clone().appendTo('.zooma-thumbnail'); // Set state for first image

  $('.product img:first-child').addClass('is-active'); // Thumbnail hover event listener
  ...
  $('.zooma-main img').on('click', function (e) {
    ...
    $(this).toggleClass('is-zoomed-in'); // Zoom in

    if ($(this).hasClass('is-zoomed-in')) {
      var calculateZoom = function calculateZoom(e) {
      ...
      var max = -(1 - 1 / scale); // Adjust mouse scale to the full-size image, then redraw

      e.offsetX *= scale;
      e.offsetY *= scale;
      calculateZoom(e); // Mouse event listener
  ...
  $('.quantity').on('click', '.btn-plus', function (event) {
    event.preventDefault(); // var qty_curr_val = $('.quantity').find('input.qty').val()
    // if (qty_curr_val==0) {
    //     return;
    // }
  ...
  $('.svg-inline--fa').find('path').removeAttr('fill'); // sticky menu
